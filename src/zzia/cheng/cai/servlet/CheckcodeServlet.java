...
/**
 * 画一张验证码图片，并将 验证码信息放到session里面，以便后面的注册、登录校验使用
 * 
 * @author SummerRain
 * 
 */
public class CheckcodeServlet extends HttpServlet {
	...
	private static final long serialVersionUID = 5655245958397993886L;
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		for (int i = 0; i < 4; i++) {
			...
			String s = String.valueOf(content.charAt(n));
			buffer.append(s);
			graphics2d.drawString(s, x, y);
			// 将角度还原一下，否则每次旋转都会在原来的基础上进行旋转
			graphics2d.rotate(-theta, x, y);
			x += 50;
		}
		// 将验证码的四个汉字存到session中
